...
First we must load in the data set that provides the supplementary data:
```{r}
library(curl)
library(tidyverse)
library(ggplot2)
...
Plots the relative abundance indices (mean of data for each Period) and associated quantiles (Descriptive analysis and 2 Inferential Analyses).
...
Mean and quantile calculation source: https://stackoverflow.com/questions/41077199/how-to-use-r-ggplot-stat-summary-to-plot-median-and-quartiles .
